...
2. **Create `<lab>/Taskfile.yml`** with the standard interface
   ...
   - Load runtime inputs from a `.env` file via a single **top-level `dotenv: ['.env']`** (committed
     `.env.example`, gitignored `.env`) — it applies to every task regardless of `dir:`. Include an
     `init-env` task that seeds `.env` from `.env.example` (no-op if `.env` exists). See the dotenv
     section in `taskfile-conventions`.
3. **Pin tools** — if missing, add `.terraform-version` and `.terragrunt-version` to the lab (read
   by tenv — a single version string each).
4. **Smoke-test cost-free** — from inside the lab folder, run `task fmt`, `task validate`,
   `task plan`. Do not run `up`/`deploy` as part of verification.
5. **Update the lab's `README.md`** with the available `task` commands.

## Acceptance

- `task --list` inside the lab folder shows the standard tasks.
