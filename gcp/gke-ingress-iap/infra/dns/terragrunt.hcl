...
#
#   phase 1 (`task gke-iap:up`):  create the child zone + write the NS delegation into the parent
#                                 zone (INGRESS_IP unset -> no A record yet).
#   phase 2 (`task gke-iap:dns`): re-apply with INGRESS_IP set to the LB IP read from the deployed
...
locals {
  ...
  # "gke-iap.gcp.example.com" -> "gke-iap-gcp-example-com".
