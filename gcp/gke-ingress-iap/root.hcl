# ---------------------------------------------------------------------------------------------------------------------
# ROOT CONFIGURATION for the gcp/gke-ingress-iap lab.
#
...
locals {
  ...
  # lab creates (e.g. gke-iap.gcp.example.com). The trailing dot is added where a FQDN is required.
  ...
  # child zone is delegated from. For gke-iap.gcp.example.com the parent is the existing
  ...
  # sign-in flow. A fully-qualified IAM member, e.g. "user:you@example.com" or
  # "group:eng@example.com". Must be a Google identity inside this organization.
