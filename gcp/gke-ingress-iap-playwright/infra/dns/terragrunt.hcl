...
#
# Two-phase by necessity (classic GKE Ingress + ManagedCertificate). The external Application Load
# Balancer's IP is EPHEMERAL — it doesn't exist until the Ingress is deployed — so the apex A record
# cannot be created at `up` time. This unit therefore:
#
#   phase 1 (`task gke-iap-pw:up`):  create the child zone + write the NS delegation into the parent
#                                    zone (INGRESS_IP unset -> no A record yet).
#   phase 2 (`task gke-iap-pw:dns`): re-apply with INGRESS_IP set to the LB IP read from the deployed
#                                    Ingress -> the apex A record is added/updated.
#
...
locals {
  ...
  # "gke-iap-pw.gcp.example.com" -> "gke-iap-pw-gcp-example-com".
