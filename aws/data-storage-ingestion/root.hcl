# ---------------------------------------------------------------------------------------------------------------------
# ROOT CONFIGURATION for the aws/streaming-vs-batch-ingestion lab.
#
...
locals {
  ...
  # Bucket that receives BOTH ingestion paths. Distinct from the state bucket.
  data_bucket = get_env("DATA_BUCKET", "REPLACE_WITH_DATA_BUCKET")
}
...
generate "provider" {
  ...
  contents  = <<-EOF
    provider "aws" {
      ...
      default_tags {
        tags = {
          Lab       = "streaming-vs-batch-ingestion"
