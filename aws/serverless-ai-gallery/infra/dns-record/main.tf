...
# the ALB's dns_name + zone_id).

variable "app_domain" {
  description = "Hostname to publish (the apex of the delegated child zone), e.g. ai-gallery.aws.example.com."
  type        = string
}

variable "hosted_zone_id" {
  description = "ID of the (child) hosted zone to create the record in."
  type        = string
}
