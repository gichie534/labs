...
# tiny unit whose outputs the `instance` unit consumes via a dependency block.
...
# Public subnets in the default VPC (default-VPC subnets auto-assign public IPs), so the instance can
# reach the SSM endpoints over the internet gateway with no NAT.
...
output "subnet_id" {
  description = "A subnet in the default VPC to launch the instance in."
